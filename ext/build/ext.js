...
(function(b) {
	...
	loadOrder: [{
		...
		requires: [19, 34, 36],
		uses: [35, 50, 257, 258, 259, 260, 261, 262, 263, 264, 265, 266, 267, 268],
		idx: 37
	}, {
		...
		requires: [20, 23, 25, 52, 74, 77, 81, 82, 83, 90, 91, 92, 107, 108, 109, 110, 111, 114, 115, 116, 117],
		uses: [1, 50, 67, 87, 194, 229, 327, 328, 329, 332, 339, 341, 411, 554, 568, 572],
		idx: 118
	}, {
		...
		requires: [135, 138, 139, 140, 141, 142, 143, 145, 146, 147, 148, 149, 150, 151, 152, 153],
		uses: [12, 156, 228],
		idx: 154
	}, {
		...
		requires: [489],
		uses: [20, 87, 154, 194, 334, 341, 416, 419, 450, 452, 455, 458, 478, 488, 534, 541, 544],
		idx: 540
	}, {
	...
	packages: {
		...
		ext: {
			...
			example: {
				...
				apps: ["admin-dashboard", "executive-dashboard", "kitchensink", "classic/charts-kitchensink", "classic/desktop", "classic/ticket-app", "classic/portal", "classic/simple-tasks", "classic/responsive-app", "classic/aria", "modern/addressbook", "modern/energy", "modern/stockapp", "modern/states", "modern/geocongress", "modern/oreilly"]
			},
			output: "${package.dir}/build",
			requires: [],
			subpkgs: ["classic/classic", "modern/modern", "packages/charts", "packages/amf", "packages/soap", "packages/ux"],
			type: "framework",
			version: "6.0.0.640"
		}
	},
...
var Ext = Ext || {};
Ext.Boot = Ext.Boot || (function(g) {
	var n = document,
		e = [],
		j = {
			disableCaching: (/[?&](?:cache|disableCacheBuster)\b/i.test(location.search) || !(/http[s]?\:/i.test(location.href)) || /(^|[ ;])ext-cache=1/.test(n.cookie)) ? false : true,
		...
		c = {
			...
			detectPlatformTags: function() {
				var p = navigator.userAgent,
					...
					v, t, u, x, o, C, s = document.createElement("div"),
					r = ["iPhone", "iPod", "Android", "Silk", "Android 2", "BlackBerry", "BB", "iPad", "RIM Tablet OS", "MSIE 10", "Trident", "Chrome", "Tizen", "Firefox", "Safari", "Windows Phone"],
					B = function(F, D) {
						if (D === undefined) {
							D = window
						}
				...
				v = (A.iPhone || A.iPod) || (!A.Silk && (A.Android && (A["Android 2"] || z))) || ((A.BlackBerry || A.BB) && A.isMobile) || (A["Windows Phone"]);
				u = (!l.isPhone) && (A.iPad || A.Android || A.Silk || A["RIM Tablet OS"] || (A["MSIE 10"] && /; Touch/.test(p)));
				x = B("touchend") || navigator.maxTouchPoints || navigator.msMaxTouchPoints;
				t = !v && !u;
				o = A["MSIE 10"];
				C = A.Blackberry || A.BB;
			...
			getHead: function() {
				return c.docHead || (c.docHead = n.head || n.getElementsByTagName("head")[0])
			},
			...
			fetch: function(o, p, x, r) {
				...
				var w = new XMLHttpRequest(),
					...
					v = function() {
						if (w && w.readyState == 4) {
							t = (w.status === 1223) ? 204 : (w.status === 0 && ((self.location || {}).protocol === "file:" || (self.location || {}).protocol === "ionp:")) ? 200 : w.status;
							u = w.responseText;
							y = {
								content: u,
								status: t,
	...
	i.prototype = {
		...
		getLoadUrl: function() {
			...
			if (!p.loadUrl) {
				p.loadUrl = !!p.buster ? (o + (o.indexOf("?") === -1 ? "?" : "&") + p.buster) : o
			}
			return p.loadUrl
		},
		fetch: function(r) {
		...
		onContentLoaded: function(p) {
			...
			t.loaded = true;
			if ((r || o === 0) && !h.phantom) {
				t.error = ("Failed loading synchronously via XHR: '" + q + "'. It's likely that the file is either being loaded from a different domain or from the local file system where cross origin requests are not allowed for security reasons. Try asynchronous loading instead.") || true;
				t.evaluated = true
			} else {
				if ((o >= 200 && o < 300) || o === 304 || h.phantom || (o === 0 && s.length > 0)) {
					t.content = s
				} else {
					t.error = ("Failed loading synchronously via XHR: '" + q + "'. Please verify that the file exists. XHR status code: " + o) || true;
					t.evaluated = true
				}
			}
		},
...
(function() {
	var b = this,
		...
		a = Ext.manifest || {},
		j, d = /\[object\s*(?:Array|Arguments|\w*Collection|\w*List|HTML\s+document\.all\s+class)\]/,
		e = /^\\?\/Date\(([-+])?(\d+)(?:[+-]\d{4})?\)\\?\/$/;
	Ext.global = b;
	...
	Ext.apply(Ext, {
		...
		isEmpty: function(i, n) {
			return (i == null) || (!n ? i === "" : false) || (Ext.isArray(i) && i.length === 0)
		},
...
Ext.Date = (function() {
	...
	return f = {
		...
		compat: function() {
			var t, u = ["useStrict", "formatCodeToRegex", "parseFunctions", "parseRegexes", "formatFunctions", "y2kYear", "MILLI", "SECOND", "MINUTE", "HOUR", "DAY", "MONTH", "YEAR", "defaults", "dayNames", "monthNames", "monthNumbers", "getShortMonthName", "getShortDayName", "getMonthNumber", "formatCodes", "isValid", "parseDate", "getFormatCode", "createFormat", "createParser", "parseCodes"],
			...
			if (s) {
				...
				for (A in m) {
					...
					if (B) {
						n = f.names;
						r = d[A];
						if (!p[n.set].$isDefault || p[n.apply] || p[n.update] || typeof r === "object") {
							if (y) {
								(e || (e = [])).push(f)
							} else {
								v.push(f);
				...
				for (A in l) {
					...
					if (!f) {
						if (x.$configStrict && typeof x.self.prototype[A] === "function") {
							Ext.raise("Cannot override method " + A + " on " + x.$className + " instance.")
						}
						x[A] = r
					} else {
						if (!f.lazy) {
		...
		reconfigure: function(s, n, t) {
			...
			for (d in n) {
				...
				if (m) {
					...
					if (r) {
						if (d !== "type") {
							Ext.log.error('No such config "' + d + '" for class ' + s.$className)
						}
						continue
					}
				}
			...
			for (g = 0, l = k.length; g < l; g++) {
				...
				if (m) {
					...
					if (s[p.set]) {
						...
						if (h) {
							if (s.$configStrict && typeof s.self.prototype[d] === "function") {
								Ext.raise("Cannot override method " + d + " on " + s.$className + " instance.")
							}
							s[d] = n[d]
						} else {
							if (d !== "type") {
								Ext.raise('Config "' + d + '" has no setter on class ' + s.$className)
							}
						}
					}
				}
...
Ext.Base = (function(c) {
	...
	h.addMembers({
		...
		callParent: function(q) {
			var s, o = (s = this.callParent.caller) && (s.$previous || ((s = s.$owner ? s : s.caller) && s.$owner.superclass[s.$name]));
			if (!o) {
		...
		callSuper: function(q) {
			var s, o = (s = this.callSuper.caller) && ((s = s.$owner ? s : s.caller) && s.$owner.superclass[s.$name]);
			if (!o) {
	...
	addMapping: function(k, e, f, m) {
		...
		for (o in h) {
			...
			for (g = 0; g < c; ++g) {
				...
				if (f[n] !== o) {
					if (f[n]) {
						Ext.log.warn("Overriding existing mapping: '" + n + "' From '" + f[n] + "' to '" + o + "'. Is this intentional?")
					}
					f[n] = o;
					l.push(n)
				}
...
Ext.ClassManager = (function(p, k, r, e, n) {
	...
	Ext.apply(Ext, {
		...
		undefine: function(y) {
			Ext.classSystemMonitor && Ext.classSystemMonitor(y, "Ext.ClassManager#undefine", arguments);
			var u = c.classes,
				v, A, t, w;
			delete u[y];
...
Ext.feature = {
	...
	tests: [{
		...
		fn: function() {
			var a = Ext.supports,
				b = 0;
			if (navigator.msMaxTouchPoints || (Ext.isWebKit && a.TouchEvents && Ext.os.is.Desktop)) {
				b = 1
			} else {
				if (a.Touch) {
					b = 2
...
Ext.define("Ext.overrides.dom.Element", (function() {
	var Element, WIN = window,
		...
		boxMarkup = ['<div class="{0}-tl" role="presentation">', '<div class="{0}-tr" role="presentation">', '<div class="{0}-tc" role="presentation"></div>', "</div>", "</div>", '<div class="{0}-ml" role="presentation">', '<div class="{0}-mr" role="presentation">', '<div class="{0}-mc" role="presentation"></div>', "</div>", "</div>", '<div class="{0}-bl" role="presentation">', '<div class="{0}-br" role="presentation">', '<div class="{0}-bc" role="presentation"></div>', "</div>", "</div>"].join(""),
		scriptTagRe = /(?:<script([^>]*)?>)((\n|\r|.)*?)(?:<\/script>)/ig,
	...
	return {
		...
		syncContent: function(source) {
			...
			var sourceNodes = source.childNodes,
				...
				i, destNode, sourceNode, nodeType, newAttrs, attLen, attName, elData = dest._extData;
		...
		privates: {
			findTabbableElements: function(options) {
				var skipSelf, skipChildren, excludeRoot, includeSaved, includeHidden, dom = this.dom,
